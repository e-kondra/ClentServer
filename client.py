...
def main():
    ...
    try:
        ...
        # прием сообщений
        THR_reseive = Thread(target=listen_message_from_server, args=(client_socket, client_name))
        THR_reseive.daemon = True
        THR_reseive.start()
        # отправка сообщений
        THR_send = Thread(target=user_interactive, args=(client_socket, client_name))
